...
def test_start_program(
    ...
    with reverts('manager: not enought amount approved'):
        rewards_contract.seed_allocations('', rewards_limit + 1, 0, {"from": balancer_allocator})

    assert rewards_contract.available_allocations() == rewards_limit
    
    rewards_contract.seed_allocations('', rewards_limit, 0, {"from": balancer_allocator})
...
def test_start_program_next_iteration(
    ...
    rewards_contract.seed_allocations('', 4 * rewards_limit, 0, {"from": balancer_allocator})
    assert rewards_contract.available_allocations() == 0
