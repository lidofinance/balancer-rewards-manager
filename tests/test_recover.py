...
def test_owner_recovers_erc20_with_zero_amount(
    balancer_wrapper, ldo_token, ldo_agent, dao_treasury
):
    rewards_amount = Wei("1 ether")
    ldo_token.transfer(balancer_wrapper, rewards_amount, {"from": dao_treasury})
    balance_before = ldo_token.balanceOf(ldo_agent)
    tx = balancer_wrapper.recover_erc20(ldo_token, 0, {"from": ldo_agent})
    assert len(tx.events) == 0

    balance_after = ldo_token.balanceOf(ldo_agent)
    assert balance_before == balance_after
    assert ldo_token.balanceOf(balancer_wrapper) == rewards_amount


def test_owner_recovers_erc20_with_balance(
    balancer_wrapper, ldo_token, ldo_agent, stranger
):
    recipient = stranger
    transfer_amount = Wei("1 ether")
    recover_amount = Wei("0.5 ether")
    ldo_token.transfer(balancer_wrapper, transfer_amount, {"from": ldo_agent})
    assert ldo_token.balanceOf(balancer_wrapper) == transfer_amount

    recipient_balance_before = ldo_token.balanceOf(recipient)
    tx = balancer_wrapper.recover_erc20(
        ldo_token, recover_amount, recipient, {"from": ldo_agent}
    )
    recipient_balance_after = ldo_token.balanceOf(recipient)

    assert ldo_token.balanceOf(balancer_wrapper) == transfer_amount - recover_amount
    assert recipient_balance_after - recipient_balance_before == recover_amount


def test_owner_recovers_erc20_to_the_caller_by_default(
    balancer_wrapper, ldo_token, ldo_agent
):
    transfer_amount = Wei("1 ether")
    ldo_token.transfer(balancer_wrapper, transfer_amount, {"from": ldo_agent})

    recipient_balance_before = ldo_token.balanceOf(ldo_agent)
    tx = balancer_wrapper.recover_erc20(ldo_token, transfer_amount, {"from": ldo_agent})
    recipient_balance_after = ldo_token.balanceOf(ldo_agent)

    assert ldo_token.balanceOf(balancer_wrapper) == 0
    assert recipient_balance_after - recipient_balance_before == transfer_amount


def test_recover_erc20_not_enough_balance(balancer_wrapper, ldo_token, ldo_agent):
    transfer_amount = Wei("1 ether")
    recover_amount = Wei("2 ether")
    ldo_token.transfer(balancer_wrapper, transfer_amount, {"from": ldo_agent})

    with reverts("token transfer failed"):
        balancer_wrapper.recover_erc20(ldo_token, recover_amount, {"from": ldo_agent})
...
def test_erc_20_recover_via_voting(
    ...
    balance = ldo_token.balanceOf(balancer_wrapper)
    assert balance > 0

    encoded_recover_calldata = balancer_wrapper.recover_erc20.encode_input(ldo_token_address, balance, stranger)
    recover_script = encode_call_script([(balancer_wrapper.address, encoded_recover_calldata)])
